...
Dato un generico nodo in cui almeno un puntatore ha valore non nullo, tale nodo prende il nome di \textbf{padre}, mentre i nodi da lui raggiungibili prendono il nome di \textbf{figli}.
...
\begin{itemize}
    \item \textbf{Caso Medio:} Se le chiavi vengono inserite in ordine casuale, l'altezza attesa dell'albero è $O(\log n)$, in quanto la struttura tende a bilanciarsi in autonomia.
...
\end{equation}
con h che indica l'altezza. Perché un albero sia AVL, deve valere che $BF(x) \in \{-1, 0, 1\}$ per ogni nodo. Grazie a questo vincolo molto stringente, gli alberi AVL sfruttano al massimo l'ampiezza. Nonostante abbiano un costo di inserimento non molto ottimizzato a causa dei frequenti controlli e delle rotazioni, la perfetta organizzazione dei dati in memoria rende questa tipologia di albero ottimale per la ricerca.
...
I test e la generazione dei grafici sono stati effettuati tramite metodi \textit{standalone} al fine di semplificare la progettazione.
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item is\_nil(node): metodo ausiliario booleano che indica se il puntatore al nodo che viene passato è nullo oppure no;
        \item get\_height(node): restituisce l'altezza del nodo passato nei parametri;
        \item update\_height(node): aggiorna l'altezza di un nodo sulla base di quella dei suoi figli;
        \item insert(key): istanzia un nuovo nodo e lo inserisce nell'albero;
        \item search(key): cerca e ritorna, se presente, il nodo la cui chiave ha lo stesso valore di quello passato;
        \item reset(): svuota un albero annullando la radice. Sfruttando il metodo ausialiario get\_nil(), è possibile renderlo indipendente dal tipo di albero che sto utilizzando;
    \end{itemize}
    ...
    \begin{itemize}
        \item get\_balance(node): calcola il fattore di bilanciamento del nodo di interesse;
        \item right\_rotate(node): gestisce la rotazione in senso orario. Il nodo passato rappresenta il perno su cui verrà eseguita la rotazione;
        ...
        \item insert\_recursive(node, key): è chiamato dal metodo \textit{insert()} e si occupa di scandire la struttura per trovare dove inserire effettivamente il nodo nell'albero.
    \end{itemize}
    ...
    \begin{itemize}
        \item get\_nil(): ridefinisce il metodo per adattarsi alle caratteristiche dell'albero;
        \item is\_nil(node): ridefinisce il metodo per adattarsi alle caratteristiche dell'albero;
...
Ciascun test viene effettuato aumentando, ad ogni iterazione, il numero di elementi da inserire o analizzare. Il numero totale delle iterazioni indica il numero di punti che comporrà ciascun grafico, e questo viene stabilito a priori impostando il valore dei parametri dedicati.
